export const dataPool = [
  // --- 公務員赴大陸 ---
  {
    fortune: "公務員赴大陸-強化措施",
    yi: "事前申請",
    ji: "隱匿行程",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/公務員赴大陸-事前申請.png",
    tip: "赴陸交流務必依規事先申請，返台後如實通報。",
  },
  {
    fortune: "公務員赴大陸-會見特定人員應記錄",
    yi: "落實登錄",
    ji: "私自接觸",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/公務員赴大陸-會見特定人士.png",
    tip: "與特定人員接觸應落實記錄。透明的行程，是公務生涯最好的保險。",
  },
  {
    fortune: "赴陸交流安全提示",
    yi: "行前報備；攜帶空白、沒有敏感機密資料的手機",
    ji: "隱蔽不報",
    comic: "https://raw.githubusercontent.com/ws20252026/drawlots/refs/heads/main/image/%E5%85%AC%E5%8B%99%E5%93%A1%E8%B5%B4%E5%A4%A7%E9%99%B8-%E6%B7%B1%E5%A4%9C%E9%81%AD%E7%9B%A4%E6%9F%A5.png", 
    tip: "近期公務員赴陸頻遭不明盤問，並被要求說明個人背景與公務經歷。提醒赴陸交流務必落實行前申報，保持警覺，切勿隨意受訪或洩漏公務機密。",
  },

  // --- 主題 2：公務機密 ---
  {
    fortune: "公務機密-守護者之盾",
    yi: "正當查詢",
    ji: "外流個資",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/公務機密-個資洩密.png",
    tip: "公務系統帳號僅限公務使用，嚴禁私用或外洩資料。",
  },
  {
    fortune: "公務機密-採購公正官靈籤",
    yi: "嚴守職守",
    ji: "洩漏底價",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/公務機密-保密之廠商投標文件案.png",
    tip: "採購底價是不可觸碰的紅線。一時「熱心」告知，可能換來法律追訴。",
  },
  {
    fortune: "公務機密-資料外洩",
    yi: "內斂修煉",
    ji: "社群打卡",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/公務機密-拍照上傳.png",
    tip: "處理機密公務時，切勿拍照上傳社群網站。魔鬼藏在細節裡，洩密常在指尖發生。",
  },
  {
    fortune: "公務機密-數位防線智慧錦囊",
    yi: "定期更換",
    ji: "便箋貼桌",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/公務機密-密碼貼桌上.png",
    tip: "電腦密碼不外流，切勿將密碼張貼於桌面或電腦周圍。守護資訊安全，就是守護自己的職涯。",
  },

  // --- 主題 3：反詐騙 ---
  {
    fortune: "反詐騙-用AI騙你錢",
    yi: "多方查證",
    ji: "急迫匯款",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/反詐騙-AI騙錢.png",
    tip: "接到『親朋好友』要錢？可能是AI!仔細看有無不合理（例如臉部模糊、不自然）。（圖源：文山第二分局偵查隊 周洛安）",
  },
  {
    fortune: "反詐騙-假文字真詐財",
    yi: "小心確認",
    ji: "點擊簡訊",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/反詐騙-假補助.gif",
    tip: "收到補助的電郵、簡訊，帳單的催繳，至公務機關官方網站、查證平台/組織查證。（圖源：文山第二分局偵查隊 周洛安）",
  },
  {
    fortune: "防詐明辨大師",
    yi: "感情可以談",
    ji: "眼見為憑",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/反詐騙-網路交友.png",
    tip: "網路世界真假難辨，不被溫柔攻勢沖昏頭。保持清醒，上網查證。（圖源：文山第二分局偵查隊 周洛安）",
  },
  {
    fortune: "不動如山清廉心",
    yi: "防詐警覺",
    ji: "貪圖利息",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/反詐騙-投資詐騙.png",
    tip: "高獲利、零風險的投資多是騙局。保持清廉心境，貪念不動，騙術不靈。",
  },

  // --- 主題 4：消費者保護 ---
  {
    fortune: "消費者保護法-正義守護卡",
    yi: "審閱契約",
    ji: "衝動消費",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/衝動消費.jpg",
    tip: "職場生存要誠信，個人消費也要明智。",
  },
  {
    fortune: "消費者保護-聰明消費智慧籤",
    yi: "官方商店下載",
    ji: "點擊不明連結",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/避免誤入詐騙網站+APP小常識.jpg",
    tip: "進入網站前請認明官方網址，讓詐騙集團無機可乘。",
  },

  // --- 主題 5：透明晶質獎 ---
  {
    fortune: "透明晶質獎",
    yi: "廉政良善治理、樹立標竿學習楷模",
    ji: "無",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/透明晶質獎簡介.jpg",
    tip: "行政透明是廉能政府的基石。讓陽光照進公務流程，激勵追求更好的廉能治理。",
  },

  // --- 陽光法案 ---
  {
    fortune: "陽光法案-職權清廉警示碑",
    yi: "利衝迴避",
    ji: "濫用圖利",
    comic: "https://raw.githubusercontent.com/ws20252026/drawlots/refs/heads/main/image/%E5%8F%8D%E8%B2%AA-%E5%88%A9%E8%A1%9D%E8%BF%B4%E9%81%BF%20(2).png",
    tip: "公職人員於執行職務過程中，擁有相關權力或裁量權，因此容易遇到有圖利自己或他人機會。未恪遵迴避規定並獲得財產上利益，將遭到裁罰。",
  },
  {
    fortune: "陽光法案-防範利衝大師",
    yi: "依法公告",
    ji: "私相補助",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/陽光法案-宮廟迴避.png",
    tip: "海派可以，違法不行！公職人員身兼民間團體（如宮廟）負責人時，該團體即屬於利衝法的「關係人」。",
  },

  // --- 反貪 ---
  {
    fortune: "反貪宣導不動如山清廉心",
    yi: "依法行政",
    ji: "接受請託",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/反貪-工程監造與不法報酬.png",
    tip: "嚴禁利用職務，貪圖不法利益。更不可假借公務刁難而主動索賄。",
  },
  {
    fortune: "清廉無瑕如意籤",
    yi: "實報實銷",
    ji: "冒名核銷",
    comic: "https://raw.githubusercontent.com/ws20252026/drawlots/refs/heads/main/image/%E5%8F%8D%E8%B2%AA-%E5%AF%A6%E5%A0%B1%E5%AF%A6%E9%8A%B7.png",
    tip: "A公所訂定年度里鄰長自強活動實施計畫，參加自強活動並親自簽到之鄰長，每人可獲得公費補助，且應專款專用。甲里長明知里內鄰長無法全數參加，竟與乙里幹事基於詐領補助經費之犯意，偽造自強活動參加名冊、繕打不實旅遊保險名單；並利用某次餐敘之機會，於會場置放空白簽到表，讓不知情鄰長簽名，藉此取得完整簽到表，事後併同相關資料送交A公所辦理核銷事宜。 里長之行為涉嫌違反《貪污治罪條例》，切勿因一時貪念自毀前程。",
  },

  // --- 反賄選 ---
  {
    fortune: "反賄選-民主基石守護卡",
    yi: "撥打檢舉賄選專線0800-024-099",
    ji: "收受賄絡",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/法務部_反賄選海報_a_a1.jpg",
    tip: "不賄選才是真賄選。",
  },
  {
    fortune: "反賄選-乾淨政治導航員",
    yi: "乾淨選風",
    ji: "旅遊招待",
    comic: "https://github.com/ws20252026/drawlots/raw/main/image/反賄選-旅遊招待.png",
    tip: "天下沒有免費的午餐！以競選宣傳請求投票支持，已涉嫌賄選罪。拒絕不正利益，守護乾淨選風。",
  },

  // --- 公益揭弊者保護法 ---
  {
    fortune: "公益揭弊者保護法-挺身守護公益",
    yi: "具名檢舉、保存事證",
    ji: "隱忍不法、知情包庇",
    comic: "https://github.com/ws20252026/drawlots/blob/main/image/%E5%85%AC%E7%9B%8A%E6%8F%AD%E5%BC%8A%E8%80%85%E4%BF%9D%E8%AD%B7%E6%B3%95%E6%91%BA%E9%A0%81%E8%83%8C%E9%9D%A2.jpg?raw=true",
    tip: "若發現泛公部門人員涉及貪污或損害公益行為，依法向受理機關具名檢舉，法律提供工作保障、人身保護與身分保密三大措施。",
  },
  //--- 行政中立 ---
  {
    fortune: "行政中立-社群媒體藏風險",
    yi: "下班使用私人設備、謹慎言論",
    ji: "上班時間、具職稱公開表態",
    comic: "https://github.com/ws20252026/drawlots/blob/main/image/%E8%A1%8C%E6%94%BF%E4%B8%AD%E7%AB%8B-%E7%A4%BE%E7%BE%A4%E5%AA%92%E9%AB%94%E8%97%8F%E9%A2%A8%E9%9A%AA.png?raw=true",
    tip: "下班時間可連結社交網站，但應避免具銜或具名發表政黨、候選人偏好言論。公務員的身分，應在網路上保持適度的專業距離。",
  },

  // --- 廉政倫理規範 ---
  {
    fortune: "飲宴應酬",
    yi: "嚴守分際、知會政風",
    ji: "接受招待、違背職務",
    comic: "https://raw.githubusercontent.com/ws20252026/drawlots/refs/heads/main/image/%E5%BB%89%E6%94%BF%E5%80%AB%E7%90%86%E8%A6%8F%E7%AF%84-%E9%A3%B2%E5%AE%B4%E6%87%89%E9%85%AC.png",
    tip: "公務員不得參加與職務有利害關係者之飲宴應酬。面對邀宴應予以拒絕，必要時應簽報長官並知會政風，守護職涯安全。",
  }
];
